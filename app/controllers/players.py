...
def get_players_route(db: Session = Depends(get_db), paid_only: bool = None, limit: int = 10):
    return player_service.get_players(db = db, paid_only=paid_only, limit=limit)
...
def get_player_route(id, db: Session = Depends(get_db)):
    ...
    player = player_service.get_player(db = db, player_id = id)
...
def update_player_route(id, player_up: PlayerUpdate, db: Session = Depends(get_db)):
    updated_player = player_service.update_player(db = db, player_id = id, player_up = player_up)
